...
public class SquareTest {
    ...
    public void testMoveOnceNormalPoint() {
        ...
        for (int i = 0; i < answers.length; i++) {
            Square moved = builder.moveOnce(cell, dirs[i], Projection.of(1,0)).get();
    ...
    public void testMoveOnceAtEdge() {
        ...
        nextCell = builder.moveOnce(cell, TwoDimension.WEST, Projection.of(1,0));
        Assert.assertFalse(nextCell.isPresent());

        nextCell = builder.moveOnce(cell, TwoDimension.SOUTHWEST, Projection.of(1,0));
        Assert.assertFalse(nextCell.isPresent());

        nextCell = builder.moveOnce(cell, TwoDimension.SOUTH, Projection.of(1,0));
        Assert.assertFalse(nextCell.isPresent());

        nextCell = builder.moveOnce(cell, TwoDimension.SOUTHEAST, Projection.of(1,0));
        Assert.assertFalse(nextCell.isPresent());
    }
}
