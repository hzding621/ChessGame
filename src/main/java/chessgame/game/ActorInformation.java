...
public class ActorInformation<C extends Cell, P extends PieceClass, B extends BoardViewer<C, P>> {
    private final Map<C, Set<Move<C>>> availableMoves = new TreeMap<>();
    ...
    public void refresh(B board, Rules<C, P, B> rules,
        ...
        board.getAllPiecesForPlayer(actor)
                ...
                .forEach(actorLocator -> {
                    ...
                    Set<Move<C>> moves = rules.basicMoves(board, actorLocator.getCell(), actor)
                            .stream()
                            .filter(targetCell ->
                            ...
                            .map(target -> SimpleMove.of(actorLocator.getCell(), target, actor))
